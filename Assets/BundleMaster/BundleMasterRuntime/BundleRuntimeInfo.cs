...
namespace BM
{
    ...
    public class BundleRuntimeInfo
    {
        ...
        public async UniTask<T> LoadAsync<T>(string assetPath) where T : UnityEngine.Object => await AssetComponent.LoadAsync<T>(assetPath, BundlePackageName);
        public UniTask<T> LoadAsync<T>(out LoadHandler loadHandler, string assetPath, bool isPool = false) where T : UnityEngine.Object => AssetComponent.LoadAsync<T>(out loadHandler, assetPath, isPool, BundlePackageName);
        public async UniTask<UnityEngine.Object> LoadAsync(string assetPath) => await AssetComponent.LoadAsync(assetPath, BundlePackageName);
        public UniTask<UnityEngine.Object> LoadAsync(out LoadHandler loadHandler, string assetPath, bool isPool = false) => AssetComponent.LoadAsync(out loadHandler,assetPath, isPool, BundlePackageName);
        public LoadSceneHandler LoadScene(string scenePath) => AssetComponent.LoadScene(scenePath, BundlePackageName);
        public async UniTask<LoadSceneHandler> LoadSceneAsync(string scenePath) => await AssetComponent.LoadSceneAsync(scenePath, BundlePackageName);
        public UniTask LoadSceneAsync(out LoadSceneHandler loadSceneHandler, string scenePath) => AssetComponent.LoadSceneAsync(out loadSceneHandler, scenePath, BundlePackageName);
