...
namespace BM
{
    public class LoadHandler : LoadHandlerBase
    {
        ...
        /// 异步等待加载的Task
        /// </summary>
        internal List<UniTaskCompletionSource> AwaitUniTasks = new List<UniTaskCompletionSource>();
        ...
        internal async UniTask LoadAsync()
        {
            ...
            LoadAsyncLoader(_loadBase, tcs).Forget();
            for (int i = 0; i < _loadDepends.Count; i++)
            {
                LoadAsyncLoader(_loadDepends[i], tcs).Forget();
            }
            for (int i = 0; i < _loadDependFiles.Count; i++)
            {
                LoadAsyncLoader(_loadDependFiles[i], tcs).Forget();
            }
            for (int i = 0; i < _loadDependGroups.Count; i++)
            {
                LoadAsyncLoader(_loadDependGroups[i], tcs).Forget();
            }
            await tcs.Task;
