...
namespace BM
{
    public class LoadSceneHandler : LoadHandlerBase
    {
        ...
        /// 异步加载场景的Bundle
        /// </summary>
        public async UniTask LoadSceneBundleAsync(UniTaskCompletionSource finishTask)
        {
            ...
            LoadAsyncLoader(_loadBase, finishTask).Forget();
            for (int i = 0; i < _loadDepends.Count; i++)
            {
                _loadDepends[i].OpenProgress();
                LoadAsyncLoader(_loadDepends[i], finishTask).Forget();
            }
            for (int i = 0; i < _loadDependFiles.Count; i++)
            {
                _loadDependFiles[i].OpenProgress();
                LoadAsyncLoader(_loadDependFiles[i], finishTask).Forget();
            }
            for (int i = 0; i < _loadDependGroups.Count; i++)
            {
                _loadDependGroups[i].OpenProgress();
                LoadAsyncLoader(_loadDependGroups[i], finishTask).Forget();
            }
