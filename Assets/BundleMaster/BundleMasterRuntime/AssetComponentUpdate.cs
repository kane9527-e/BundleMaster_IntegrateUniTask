...
namespace BM
{
    public static partial class AssetComponent
    {
        ...
        /// </summary>
        private static async UniTask CalcNeedUpdateBundle(UpdateBundleDataInfo updateBundleDataInfo, string bundlePackageName, string remoteVersionLog, string localVersionLog)
        {
...
#endif
            UniTaskCompletionSource streamingLogTcs = new UniTaskCompletionSource();
            string streamingLog;
            using (UnityWebRequest webRequest = UnityWebRequest.Get(streamingAssetLogPath))
            {
                ...
                weq.completed += (o) =>
                {
                    streamingLogTcs.TrySetResult();
                };
                await streamingLogTcs.Task;
#if UNITY_2020_1_OR_NEWER
                if (webRequest.result != UnityWebRequest.Result.Success)
#else
                if (!string.IsNullOrEmpty(webRequest.error))
        ...
        /// </summary>
        private static async UniTask CalcNeedUpdateBundleFileCRC(UpdateBundleDataInfo updateBundleDataInfo, string bundlePackageName, string remoteVersionLog, string localVersionLog)
        {
            string[] remoteVersionData = remoteVersionLog.Split('\n');
            string[] localVersionData = localVersionLog.Split('\n');
            string[] remoteVersionDataSplits = remoteVersionData[0].Split('|');
            ...
            for (int i = 0; i < loop; i++)
            {
                ...
                UniTaskCompletionSource finishTcs = new UniTaskCompletionSource();
                for (int j = 0; j < count; j++)
                {
                    CheckFileCRC(remoteVersionData[i * loopSize + j + 1], bundlePackageName, needUpdateBundles, finishTcs).Forget();
                }
        ...
        public static async UniTask DownLoadUpdate(UpdateBundleDataInfo updateBundleDataInfo)
        {
            ...
            UniTaskCompletionSource downLoading = new UniTaskCompletionSource();
            //准备需要下载的内容的初始化信息
            ...
            for (int i = 0; i < AssetComponentConfig.MaxDownLoadCount; i++)
            {
                foreach (Queue<DownLoadTask> downLoadTaskQueue in packageDownLoadTask.Values)
                {
                    ...
                    {
                        downLoadTaskQueue.Dequeue().DownLoad().Forget();
                        break;
                    }
                }
            }
    ...
    public class DownLoadTask
    {
        ...
        public UniTaskCompletionSource DownLoadingKey;
