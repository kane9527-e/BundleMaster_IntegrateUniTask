...
namespace BM
{
    public partial class LoadBase
    {
        ...
        /// 加载完成后需要执行的Task
        /// </summary>
        private List<UniTaskCompletionSource> _loadFinishTasks = new List<UniTaskCompletionSource>();
        ...
        internal void LoadAssetBundle(string bundlePackageName)
        {
            ...
            {
                _loadFinishTasks[i].TrySetResult();
            }
            _loadFinishTasks.Clear();
        }
    
        internal async UniTaskVoid LoadAssetBundleAsync(UniTaskCompletionSource tcs, string bundlePackageName)
        {
            AddRefCount();
            if (_loadState == LoadState.Finish)
            {
